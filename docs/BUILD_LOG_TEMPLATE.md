...
2. **What We Accomplished Today** — review the git commits for today (`git log --oneline -10` + `git show --stat HEAD`) to determine what was worked on in today's session. Write everything that was done today, and include where we left off. Be specific enough that the next session can pick up cleanly without additional context. Confirm with the user if anything is unclear.
